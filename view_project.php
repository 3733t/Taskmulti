...
$stmt = $conn->prepare("SELECT * FROM project_list where id = :id");
...
$stmt = $conn->prepare("SELECT *,concat(firstname,' ',lastname) as name FROM users where id = :manager_id");
      ...
      <div class="card card-outline card-primary">
        ...
        <div class="card-body">
          <ul class="users-list clearfix">
            <?php 
            if(!empty($user_ids)){
              $stmt = $conn->prepare("SELECT *,concat(firstname,' ',lastname) as name FROM users where id in ($user_ids) order by concat(firstname,' ',lastname) asc");
              $stmt->execute();
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="card">
        ...
        <div class="card-body">
          <?php 
          $stmt = $conn->prepare("SELECT p.*,concat(u.firstname,' ',u.lastname) as uname,u.avatar,t.task FROM user_productivity p inner join users u on u.id = p.user_id inner join task_list t on t.id = p.task_id where p.project_id = :project_id order by unix_timestamp(p.date_created) desc");
          $stmt->bindValue(':project_id', $id);
          $stmt->execute();
          while($row = $stmt->fetch(PDO::FETCH_ASSOC)):
          ?>
