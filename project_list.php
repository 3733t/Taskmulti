        ...
        <div class="card-body">
            <table class="table tabe-hover table-condensed" id="list">
                ...
                <tbody>
                    ...
                    $stmt = $conn->query("SELECT * FROM project_list $where order by name asc");
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)):
                        ...
                        $tprog = $conn->query("SELECT * FROM task_list where project_id = {$row['id']}")->rowCount();
                        $cprog = $conn->query("SELECT * FROM task_list where project_id = {$row['id']} and status = 3")->rowCount();
                        $prog = $tprog > 0 ? ($cprog / $tprog) * 100 : 0;
                        $prog = $prog > 0 ? number_format($prog, 2) : $prog;
                        $prod = $conn->query("SELECT * FROM user_productivity where project_id = {$row['id']}")->rowCount();
