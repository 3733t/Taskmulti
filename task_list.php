			...
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					...
					if ($_SESSION['login_type'] == 2) {
						$where = " where p.manager_id = :manager_id ";
					} elseif ($_SESSION['login_type'] == 3) {
						$where = " where concat('[',REPLACE(p.user_ids,',','],['),']') LIKE :user_id ";
					}

					$stat = array("Pending", "Started", "On-Progress", "On-Hold", "Over Due", "Done");
					$query = "SELECT t.*, p.name as pname, p.start_date, p.status as pstatus, p.end_date, p.id as pid FROM task_list t INNER JOIN project_list p ON p.id = t.project_id $where ORDER BY p.name ASC";
